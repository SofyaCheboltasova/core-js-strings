...
function isPalindrome(str) {
  const cleared = str.toLowerCase().replace(/[^a-z0-9]/g, '');
...
function extractNameFromTemplate(value) {
  ...
  return value.slice(startIdx + 1, endIdx);
}
...
function encodeToRot13(str) {
  const alphabetLower = 'nopqrstuvwxyzabcdefghijklm';
  const alphabetUpper = 'NOPQRSTUVWXYZABCDEFGHIJKLM';
  let result = '';
  let alphabet;
  let index;
...
function getCardId(value) {
  ...
  const suitI = suits.indexOf(value.charAt(value.length - 1));
  const numberI = numbers.indexOf(value.slice(0, -1));

  return suitI * numbers.length + numberI;
}
